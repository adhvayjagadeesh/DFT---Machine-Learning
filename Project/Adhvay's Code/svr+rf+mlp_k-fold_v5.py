...
# Load dataset
df = pd.read_csv("c2db_rectangular-lattice-structure_materials.csv")
...
pred_matrix = np.vstack([svr_all, rf_all, mlp_all]).T
...
pred_holdout_matrix = np.vstack([svr_holdout, rf_holdout, mlp_holdout]).T
...
print(f"Hybrid Model MAE (Holdout Set): {mae:.4f}")
print(f"Hybrid Model R² (Holdout Set): {r2:.4f}")
