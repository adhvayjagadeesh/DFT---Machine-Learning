...
print(f"Bootstrapped R²: {np.mean(r2_scores):.4f} ± {np.std(r2_scores):.4f}")

# Plot regression graph (last model's predictions)
plt.scatter(y_test, final_y_pred, alpha=0.5)
plt.xlabel("Actual Band Gap (HSE06) [eV]")
plt.ylabel("Predicted Band Gap (HSE06) [eV]")
plt.title("Actual vs Predicted Band Gap (HSE06) [Last Bootstrap Model]")
