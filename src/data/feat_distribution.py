...
parser = ArgumentParser("Feature distribution histograms", description="Show how features are distributed")
parser.add_argument("-s", "--save", help="Save the figure(s) instead of showing it")
args = parser.parse_args()

# All features + HSE06 band gap
data = itertools.chain(x.drop(columns=["Formula", "Magnetic"]).items(), y.to_frame().items())
...
for i, (name, vals) in enumerate(data):
  ...
  ax_box.boxplot(vals, vert=False, widths = 0.75, patch_artist = True,
